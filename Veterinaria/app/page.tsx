...
import { Usuario, Cartilla_Vacunacion, Mascotas } from '@/lib/types'
import { servicios, mascotas as initialMascotas, cartillas as initialCartillas, getNombreCompleto } from '@/lib/mock-data'
...
export default function MediVetApp() {
  ...
  const [cartillas, setCartillas] = useState<Cartilla_Vacunacion[]>(initialCartillas)
  const [mascotas, setMascotas] = useState<Mascotas[]>(initialMascotas)
  ...
  const handleLogin = useCallback((user: Usuario) => {
    ...
    toast.success(`Bienvenido/a, ${getNombreCompleto(user.id)}`)
  }, [])
  ...
  const handleCreateCartilla = useCallback((cartilla: Omit<Cartilla_Vacunacion, 'id'>) => {
    const newCartilla: Cartilla_Vacunacion = {
      ...cartilla,
      id: cartillas.length + 1
    }
    setCartillas(prev => [...prev, newCartilla])
    toast.success('Cartilla creada exitosamente')
  }, [cartillas.length])

  const handleUpdateCartilla = useCallback((updatedCartilla: Cartilla_Vacunacion) => {
    setCartillas(prev => 
      prev.map(c => c.id === updatedCartilla.id ? updatedCartilla : c)
    )
    toast.success('Cartilla actualizada exitosamente')
  }, [])

  const handleAgregarMascota = useCallback((mascota: Omit<Mascotas, 'id'>) => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'veterinarios_lista':
        return <ListaVeterinariosView />

      case 'citas': 
        return <CitasView />

      case 'settings':
        return <SettingsView userName={currentUser ? getNombreCompleto(currentUser.id) : 'Usuario'} />
